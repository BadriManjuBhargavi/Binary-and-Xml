...
namespace Assignment24
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            using (FileStream file = new FileStream("C:\\DAY 21\\employee.bin", FileMode.Create))
            {
                binaryFormatter.Serialize(file, emp);
            }
            Console.WriteLine("Binary File Serialized");
            Employee DeEmp;
            using (FileStream file = new FileStream("C:\\DAY 21\\employee.bin", FileMode.Open))
            {
            ...
            using (FileStream file = new FileStream("C:\\DAY 21\\employee.xml", FileMode.Create))
            {
                xml.Serialize(file, employee);
            }
            Console.WriteLine("XMl File Serialized");
            Employee DeEmployee;
            using (FileStream file = new FileStream("C:\\DAY 21\\employee.xml", FileMode.Open))
            {
